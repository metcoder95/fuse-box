...
class FallbackWorkflow {
  constructor ({ value, handleAbort = false }) {
    ...
    this.isPrimitive = typeof value !== 'function'
    this.isAsync =
      this.isPrimitive === false &&
      FunctionConstructor !== value?.constructor?.name &&
...
export function Fallback (options) {
  if (options.value == null) {
    throw new Error('Fallback workflow requires a fallback value')
  }

  if (options.handleAbort != null && typeof options.handleAbort !== 'boolean') {
    throw new Error('Fallback workflow "handleAbort" option must be a boolean')
  }
