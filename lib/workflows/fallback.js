...
class FallbackWorkflow {
  constructor ({ fallback, handleAbort = false }) {
    this.fallback = fallback
    this.handleAbort = handleAbort
    this.isPrimitive = FunctionConstructor !== fallback?.constructor?.name
    this.isAsync =
      this.isPrimitive === false &&
      AsyncFunctionConstructor === fallback?.constructor?.name
  }
